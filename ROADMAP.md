...
> **현재 상태**: MVP 코드 구현 완료(Task 006–017). 남은 것은 **Task 018 — 사용자 준비물(Supabase 프로젝트·KIS 키·Vercel 계정) 확보 후 실데이터 백필·검증·배포**. Task 010(환율)은 코드 완성이나 `FID_INPUT_ISCD` 값을 실계좌 dry-run으로 최종 확정해야 한다(실패 시 ECOS 교체).
...
### Task 010: 환율 일봉 수집 (FHKST03030100) ✅ 완료 (종목코드 실계좌 확정 대기)
