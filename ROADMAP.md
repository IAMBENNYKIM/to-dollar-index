...
| Phase 4 | 운영 신뢰성 & UX 폴리시 | 022–026 | 5/5 | 🔄 런타임 설정 대기 |
| **합계** | | **26** | **25/26 (96%)** | |

> **현재 상태**: **MVP 배포 완료**(GitHub Actions 수집 + Vercel 운영). Phase 3 부동산 지표까지 운영 반영 완료. Phase 4(최신성 배지, 텔레그램 실패 알림, ECOS 환율 폴백, 홈 요약 대시보드, 모바일 반응형) 코드 완료 — 남은 것은 **사용자의 Actions Secrets 3개 등록**(`TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID`/`ECOS_API_KEY`, 미등록 시에도 기존 동작은 그대로 유지). 다음 개발 항목은 Phase 2 최저임금(Task 019).
...
- **남은 런타임 작업(사용자)**: @BotFather 봇 생성 → Secrets `TELEGRAM_BOT_TOKEN`·`TELEGRAM_CHAT_ID` 등록.
...
- **남은 런타임 작업(사용자)**: ECOS 인증키 발급 → Secret `ECOS_API_KEY` 등록.
