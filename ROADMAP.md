...
| Phase 1 | MVP (환율 + 국내주식) | 006–018 | 0/13 | 🔄 진행 중 |
| Phase 2 | 최저임금 | 019 | 0/1 | ⬜ 대기 |
| Phase 3 | 부동산 | 020–021 | 0/2 | ⬜ 대기 |
| **합계** | | **21** | **5/21 (24%)** | |
...
### Task 006: KIS 토큰 발급·캐싱 (kis_auth.py) 🔄 진행 중
...
### Task 007: Supabase writer (database_writer.py) 🔄 진행 중
...
### Task 008: 백필 구간 분할 유틸 (date_windows.py) 🔄 진행 중
...
### Task 009: 국내주식 일봉 수집 (FHKST03010100, 수정주가) ⬜ 대기
...
### Task 010: 환율 일봉 수집 (FHKST03030100) ⬜ 대기
...
### Task 011: 수집 CLI 통합 (backfill / daily / --dry-run) ⬜ 대기
...
### Task 012: GitHub Actions 수집 워크플로 (일일 + 월간 재백필·keepalive) ⬜ 대기
...
### Task 013: 프론트 Supabase 클라이언트·쿼리 ⬜ 대기
...
### Task 014: 구간 수익률 계산 함수 (rangeReturns.ts + vitest) ⬜ 대기
...
### Task 016: 지표 상세 원화/달러 이중 차트 + dataZoom 구간 수익률 ⬜ 대기
...
### Task 017: 지표 내비게이션·카드 목록 → MVP 완성 ⬜ 대기
...
### Task 018: 실데이터 검증 + Vercel 배포 ⬜ 대기
- 산출물: Supabase 프로젝트(운영), Vercel 배포(Root Directory = `web`), 검증 결과 기록
- 의존: 012, 017
- 완료 기준: Supabase 운영 프로젝트 생성 → 마이그레이션 적용 → 백필 실행 → **수집 종가와 공개 종가 표본 대조 일치**. RLS 동작 확인(anon 쓰기 거부). Vercel 배포 후 첫 화면·상세·구간 수익률이 실데이터로 정상 렌더 = **MVP 배포 완료**.
...
**마일스톤: 서울 구별 월평균 실거래가의 원화/달러 비교 제공.**

### Task 020: 국토부 실거래가 수집기 ⬜ 대기
- 산출물: `collector/src/collector/real_estate.py`, `.github/workflows` 수집 스텝 확장
- 의존: Phase 1 (collector 인프라)
- 완료 기준: 국토부 아파트 실거래가 API(`apis.data.go.kr`) 조회 → 구별 월평균 집계 → `real_estate_monthly` upsert. **최근 3개월은 매 실행 재수집·재집계**(신고 지연 30일 대응). 멱등 보장.

### Task 021: 부동산 구별 월평균 원화/달러 페이지 ⬜ 대기
- 산출물: `web/src/app/real-estate/page.tsx`, `web/src/components/RealEstateChart.tsx`
- 의존: 020
- 완료 기준: 서울 특정 구의 월평균 집값 원화 값 + 달러 환산 값을 이중 차트로 비교. Phase 1의 dataZoom 구간 수익률 로직(Task 014) 재사용.
